...
class ANB:
    ...
    def test_oct(self, epoch, plot_hist=True):
        with torch.no_grad():
            self.opt.phase = 'test'
            means = torch.rand(
                size=(5, len(self.dataloader["gen"][0].valid.dataset), self.opt.n_MC_Gen, self.opt.n_MC_Disc),
                dtype=torch.float32,
                device=self.device)
            ...
            for patch_idx in range(5):
                for _idxData, (x_real, label) in enumerate(self.dataloader["gen"][0].valid, 0):
                    x_real = x_real.to(self.device)[:, patch_idx, ...]
                    if patch_idx == 0:
                        gt_labels[_idxData * self.opt.batchsize: _idxData * self.opt.batchsize + label.size(0)] = label
                    ...
                    for _idxD in range(self.opt.n_MC_Disc):
                        ...
                        for _idxG in range(self.opt.n_MC_Gen):
                            ...
                            means[patch_idx, _idxData * self.opt.batchsize:(_idxData + 1) * self.opt.batchsize, _idxG, _idxD].copy_(
                                lat)
            means = torch.sum(means, dim=0, keepdim=False)
